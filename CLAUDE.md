...
**框架**：后端为 **FastAPI + uvicorn**（2026-07 从 Flask 迁移；端点用同步 def 走线程池，SSE 用 StreamingResponse）。
**端口**：后端默认 **5001**（macOS 5000 常被 AirPlay 占用；可用 `PORT` 覆盖），前端 5173，Vite 代理 `/api → 5001`。
**配置**：**API Key 等配置存 SQLite `app_settings` 表（cfg_ 前缀），是唯一事实来源**；`backend/.env` 仅在首次启动播种进 DB（`db.apply_settings_overlay()`）。修改配置走「系统设置」页或 `POST /api/settings`（`db.set_config` 写 DB + 热更新 config 模块）。
