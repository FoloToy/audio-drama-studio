    ...
    <div className="text-sm space-y-0.5 max-h-[28rem] overflow-y-auto pr-1">
      {script.map((item, i) => {
        ...
        if (item.type === 'bgm') {
          ...
          if (isStop) return (
            <div key={i} className="px-2 py-0.5 text-xs text-th-xlo italic select-none">
              ▪ BGM 停止
            </div>
          )
          return (
              ...
              {editable ? (
                <input
                  className="flex-1 bg-transparent border-b border-[#E5007F]/30 outline-none
                    focus:border-[#FF3BA8] text-xs min-w-0 text-[#FF70BF]"
                  value={item.name ?? ''}
        ...
        if (item.type === 'tts') {
          ...
          return (
            <div key={i} className={`flex gap-2 py-0.5 ${editable ? 'items-start' : 'items-baseline'}`}>
              ...
              {editable ? (
                <textarea
                  className="flex-1 text-th-md leading-relaxed resize-none text-xs
                ...
                <span className="text-th-md leading-relaxed">{item.text}</span>
              )}
            </div>
          )
        }
