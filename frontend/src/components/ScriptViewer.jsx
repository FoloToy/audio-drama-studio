    ...
    <div className="text-sm space-y-0.5 max-h-[28rem] overflow-y-auto pr-1">
      {script.map((item, i) => {
        ...
        if (item.type === 'bgm') {
          ...
          if (isStop) return (
            <div key={i} className="px-2 py-0.5 text-xs text-gray-400 italic select-none">
              ▪ BGM 停止
            </div>
          )
          return (
            <div key={i} className="flex items-center gap-1 px-2 py-0.5 rounded text-xs
              ...
              {editable ? (
                <input
                  className="flex-1 bg-transparent border-b border-purple-200 outline-none
                    focus:border-purple-400 text-xs min-w-0"
                  value={item.name ?? ''}
        ...
        if (item.type === 'sfx') {
          return (
            <div key={i} className="flex items-center gap-1 px-2 py-0.5 rounded text-xs
              ...
              {editable ? (
                <input
                  className="flex-1 bg-transparent border-b border-amber-200 outline-none
                    focus:border-amber-400 text-xs min-w-0"
                  value={item.name ?? ''}
                  onChange={e => update(i, { name: e.target.value })}
                />
              ) : (
          ...
          return (
            <div key={i} className={`flex gap-2 py-0.5 ${editable ? 'items-start' : 'items-baseline'}`}>
              <span className={`shrink-0 font-medium w-14 text-right text-xs leading-6
                ${isNarrator ? 'text-gray-400' : 'text-indigo-600'}`}>
                {item.speaker}：
              </span>
              {editable ? (
                <textarea
                  className="flex-1 text-gray-700 leading-relaxed resize-none text-xs
                    border border-transparent rounded px-1.5 py-0.5
                    hover:border-gray-200 focus:border-indigo-300 focus:outline-none
                    focus:ring-1 focus:ring-indigo-200 bg-white transition-colors"
                  value={item.text ?? ''}
